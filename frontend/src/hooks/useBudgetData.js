...
export const useBudgetData = () => {
    ...
    const handleFormSubmit = async () => {
        ...
        const isSplit = formData.type === 'expense' && formData.split_with && formData.split_with.length > 0;
        let finalPerson = null;

        // Determine the person only if it's relevant for the category and not a split
        if (!isSplit && personCategories.includes(finalCategory)) {
            if (formData.person === 'add_new') {
                ...
                finalPerson = formData.newPerson.trim();
            } else {
                finalPerson = formData.person;
            }
        }
    ...
    const handleSettleUp = async (person, account_id) => {
        ...
        try {
            ...
            // Optimized Refresh: Fetch only what's changed
            const [peopleStatsData, dashboardStatsData, splitSummariesData] = await Promise.all([
                api.stats.getPeopleStats(selectedAccountId),
                api.stats.getDashboardStats(selectedAccountId),
                api.stats.getSplits(selectedAccountId)
            ]);
            ...
            setPeopleStats(peopleStatsData);
            setStats({ 
                totalIncome: dashboardStatsData.total_income || 0, 
                totalExpenses: dashboardStatsData.total_expenses || 0, 
                balance: dashboardStatsData.balance || 0, 
                transactionCount: dashboardStatsData.transaction_count || 0 
            });
            setSplitSummaries(splitSummariesData);
            
            // Now, just refetch the transaction list
            fetchTransactions();
    ...
    const handleEditClick = (transaction) => {
        ...
        // This logic is complex because the API doesn't tell us what the base categories are.
        // We simulate it by checking against a hardcoded list, which is not ideal but works.
        const knownIncomeCategories = ['Salary', 'Freelance', 'Investments', 'From Friends', 'From Parents', 'Settlement'];
        const knownExpenseCategories = ['Food', 'Transportation', 'Rent', 'Utilities', 'Shopping', 'Entertainment', 'Miscellaneous', 'To Friends', 'Settlement'];

        const isStandardIncome = knownIncomeCategories.includes(transaction.category);
        const isStandardExpense = knownExpenseCategories.includes(transaction.category);
