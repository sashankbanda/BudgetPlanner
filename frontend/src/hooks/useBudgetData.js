...
import { useDebounce } from './useDebounce'; // Import the new debounce hook
...
    
    // Loading states
    const [loading, setLoading] = useState(true); // For initial dashboard load
    const [isTransactionLoading, setIsTransactionLoading] = useState(true); // For transaction list only
...
    const debouncedSearchTerm = useDebounce(filters.search, 300); // Debounce search input
...
    // 1. Fetches core dashboard data (everything except transactions)
    const loadCoreData = useCallback(async () => {
        // Don't set main loading state if it's just a refresh
...
            const [
                accountsData, dashboardStats, incomeStats, 
                expenseStats, trendStats, peopleData, peopleStatsData
            ] = await Promise.all([
                api.accounts.getAll(),
...
    // Initial load and subsequent reloads of core data
    useEffect(() => {
        loadCoreData();
    }, [loadCoreData]);

    // 2. Fetches ONLY transactions when filters or account change
    useEffect(() => {
        // Avoid fetching transactions until the initial core data load is complete
        if (loading) return;
...
            setNewAccountName("");
            // Set new account as selected, which will trigger `loadCoreData` automatically via useEffect
...
            setSelectedAccountId('all'); // This will trigger a reload
        } catch (error) {
...
            type: formData.type, category: finalCategory, amount: parseFloat(formData.amount),
            description: formData.description, date: formData.date, person: formData.person || null,
...
            loadCoreData(); // Reload all dashboard data after a transaction
        } catch (error) {
...
            loadCoreData(); // Reload all dashboard data
        } catch (error) {
...
    };
    
    // --- MEMOIZED VALUES & OTHER LOGIC ---
    const handleFilterChange = (key, value) => setFilters(prev => ({ ...prev, [key]: value }));

    // ✨ FIX: Implemented the resetForm function
    const resetForm = useCallback(() => {
    ...
    }, [accounts]); // Dependency on accounts to set the default account_id correctly

    // ✨ FIX: Implemented the handleEditClick function
    const handleEditClick = (transaction) => {
        setEditingTransaction(transaction);
        
        // Check if the category is a standard one or custom
    ...
    // ✨ FIX: Implemented the handleDeleteClick function
