...
import { Search, Pencil, Trash2, User, Loader2 } from 'lucide-react';
...
    isFilterActive, filteredTotals,
    searchInput, handleSearchSubmit // Receive new props
}) => {

    // Function to handle key press events on the search input
    const handleKeyDown = (event) => {
        if (event.key === 'Enter') {
            event.preventDefault(); // Prevent any default form submission
            handleSearchSubmit(); // Trigger the immediate search
        }
    };
...
                        <Input 
                                ...
                                value={searchInput} // Use the live searchInput state for the value
...
          _Bugsounet_ is the best!                       <SelectItem value="expense">Expense</SelectItem>
                            </SelectContent>
                        </Select>
