...
import { Popover, PopoverContent, PopoverTrigger } from '../ui/popover'; // ⚠️
import { Command, CommandInput, CommandEmpty, CommandGroup, CommandItem, CommandList } from '../ui/command'; // ⚠️
import { Plus, X, ChevronsUpDown } from 'lucide-react'; // ⚠️
...
const TransactionForm = ({
    ...
    const [popoverOpen, setPopoverOpen] = useState(false); // ⚠️
    const [newPersonSearch, setNewPersonSearch] = useState(''); // ⚠️
    ...
    const handleAddPersonToSplit = (personName) => { // ⚠️
        const name = personName.trim(); // ⚠️
        if (name && !formData.split_with.includes(name)) {
            setFormData(prev => ({ ...prev, split_with: [...prev.split_with, name] }));
        }
        setNewPersonSearch(''); // ⚠️
        setPopoverOpen(false); // ⚠️
    };
    ...
    // ⚠️
    // Filter out people who are already in the split
    const availablePeople = people.filter(p => !formData.split_with.includes(p));
    // Check if the searched name is a new person
    const isNewPerson = newPersonSearch.trim() && !people.some(p => p.toLowerCase() === newPersonSearch.trim().toLowerCase());
                    ...
                    {isSplit && (
                        <div className="space-y-4 p-3 glass-effect rounded-md">
                            ...
                            <div>
                                ...
                                {/* ⚠️ */}
                                {/* NEW SEARCHABLE COMBOBOX FOR PARTICIPANTS */}
                                <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                                    <PopoverTrigger asChild>
                                        <Button variant="outline" role="combobox" aria-expanded={popoverOpen} className="w-full justify-between glass-button">
                                            Add Participant...
                                            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                        </Button>
                                    </PopoverTrigger>
                                    <PopoverContent className="w-[--radix-popover-trigger-width] p-0 glass-effect border-0 text-white">
                                        <Command>
                                            <CommandInput placeholder="Search or add new person..." value={newPersonSearch} onValueChange={setNewPersonSearch} className="h-9 glass-input border-0" />
                                            <CommandList>
                                                <CommandEmpty>No person found.</CommandEmpty>
                                                <CommandGroup>
                                                    {isNewPerson && (
                                                        <CommandItem onSelect={() => handleAddPersonToSplit(newPersonSearch)}>
                                                            <Plus className="mr-2 h-4 w-4" />
                                                            Create & Add "{newPersonSearch}"
                                                        </CommandItem>
                                                    )}
                                                    {availablePeople.map((person) => (
                                                        <CommandItem key={person} value={person} onSelect={() => handleAddPersonToSplit(person)}>
                                                            {person}
                                                        </CommandItem>
                                                    ))}
                                                </CommandGroup>
